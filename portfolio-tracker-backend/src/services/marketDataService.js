...
const indianStockApiService = require("./indianStockApiService");
...
const marketDataService = {
  ...
  // Symbol to company name mapping for Indian Stock API
  getApiCompanyName(symbol) {
    const symbolToApiName = {
      RELIANCE: "RELIANCE",
      TCS: "TCS",
      HDFCBANK: "HDFC BANK",
    ...
    return symbolToApiName[baseSymbol] || null;
  },
  async getStockQuote(symbol) {
    try {
      // Try to get real data from Indian Stock API first
      try {
        const apiCompanyName = this.getApiCompanyName(symbol);
        if (apiCompanyName) {
          ...
          if (stockData && (stockData.currentPrice || stockData.price)) {
            return {
              symbol: symbol,
              name: stockData.companyName || this.getCompanyName(symbol),
              price: parseFloat(stockData.currentPrice || stockData.price) || 0,
              change: parseFloat(stockData.change || stockData.netChange) || 0,
              changePercent:
                parseFloat(
                  stockData.changePercent || stockData.percentChange
                ) || 0,
              high: parseFloat(stockData.high || stockData.yearHigh) || 0,
              low: parseFloat(stockData.low || stockData.yearLow) || 0,
              open: parseFloat(stockData.open) || 0,
              previousClose:
                parseFloat(stockData.previousClose || stockData.close) || 0,
              volume: parseInt(stockData.volume) || 0,
              marketCap: stockData.marketCap || null,
              exchange:
                stockData.exchange || (symbol.includes(".BSE") ? "BSE" : "NSE"),
        ...
        logger.warn(
          `Indian Stock API failed for ${symbol} (${this.getApiCompanyName(
            symbol
      ...
      // Check if symbol is in our known symbols list before providing fallback
      const baseSymbol = symbol.replace(/\.(NSE|BSE)$/, "");
      const knownSymbols = [
        "RELIANCE",
        "TCS",
      ...
      // Fallback to mock data for known symbols only
  ...
  async getTrendingStocks() {
    try {
      // Try to get real trending data from Indian Stock API first
      try {
        const trendingData = await indianStockApiService.getTrendingStocks();
        if (
          trendingData &&
          (trendingData.topGainers || trendingData.topLosers)
        ) {
          return {
            topGainers: trendingData.topGainers || [],
            topLosers: trendingData.topLosers || [],
            source: "indian_stock_api",
          };
        }
      } catch (apiError) {
        logger.warn(
          "Indian Stock API failed for trending stocks, falling back to mock data:",
          apiError.message
        );
      }
      ...
      return { topGainers: [], topLosers: [], source: "error" };
    }
  },
  async getNSEMostActive() {
    try {
      // Try to get real NSE data from Indian Stock API first
      try {
        ...
        logger.warn(
          "Indian Stock API failed for NSE most active, falling back to mock data:",
          apiError.message
        );
      }
  ...
  async getBSEMostActive() {
    try {
      // Try to get real BSE data from Indian Stock API first
      try {
        const bseData = await indianStockApiService.getBSEMostActive();
        if (bseData && Array.isArray(bseData)) {
          return bseData.map((stock) => ({
            symbol: stock.symbol,
            name: stock.name || stock.symbol,
            price: parseFloat(stock.price) || 0,
            change: parseFloat(stock.change) || 0,
        ...
        logger.warn(
          "Indian Stock API failed for BSE most active, falling back to mock data:",
          apiError.message
        );
      }
  ...
  async getHistoricalData(
    ...
    try {
      // Check if symbol is in our known symbols list before providing data
      const baseSymbol = symbol.replace(/\.(NSE|BSE)$/, "");
      const knownSymbols = [
        "RELIANCE",
        "TCS",
  ...
  async getPriceHistory(
    ...
    try {
      return await this.getHistoricalData(symbol, period, startDate, endDate);
    } catch (error) {
      console.error(
